...
public class ProfileUpdatedAspect {
  ...
  @Pointcut("within(fi.okm.jod.ohjaaja.service.profiili.*) && args(user,..)")
    ...
    if (isSynchronizationActive() && !isCurrentTransactionReadOnly()) {
      ...
      registerSynchronization(
          new SynchronizationCallback(
              entityManager.getReference(Ohjaaja.class, user.getId()), operation));
    }
    ...
    public void beforeCommit(boolean readOnly) {
      if (!readOnly) {
        log.atInfo()
            ...
            .log("User {} profile updated", ohjaaja.getId());
      }
    }
  }
}
